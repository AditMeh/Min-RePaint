...
def main():
    ...
    checkpoint = sorted(os.listdir("checkpoints_celeba/"),
    ...
    unet = UNet2DModel(sample_size=(128, 128), in_channels=3,
                       out_channels=3).to(device)

    unet.load_state_dict(torch.load(
        f'./checkpoints_celeba/{last_checkpoint}', map_location=device))
    
    print("Loaded: ", f'./checkpoints_celeba/{last_checkpoint}')
